...
CLCommandQueue.prototype.enqueueSVMMap = function(blocking_map, map_flags, svm_ptr, size, event_wait_list) {
    this._throwIfReleased();
    var event = this.isWaitable ? ref.alloc(this.cl.types.Event) : null;
    var waitList = this._toWaitList(event_wait_list);
    var err = this.cl.imports.clEnqueueSVMMap(this.handle, false, map_flags, clUtils.toHandle(svm_ptr), size, waitList.count, waitList.array, event);
    this.cl.checkError(err);
    if (event) {
        return new CLEvent(this.cl, event.deref());
    }
...
CLCommandQueue.prototype.enqueueSVMUnmap = function(svm_ptr, event_wait_list) {
	this._throwIfReleased();
	var event = this.isWaitable ? ref.alloc(this.cl.types.Event) : null;
     var waitList = this._toWaitList(event_wait_list);
     var err = this.cl.imports.clEnqueueSVMUnmap(this.handle, clUtils.toHandle(svm_ptr), waitList.count, waitList.array, event);
     this.cl.checkError(err);
     if (event) {
         return new CLEvent(this.cl, event.deref());
     }
 ...
 CLCommandQueue.prototype.enqueueSVMFree = function(num_svm_pointers, svm_pointers, pfn_free_func, user_data, event_wait_list) {
     this._throwIfReleased();
     var event = this.isWaitable ? ref.alloc(this.cl.types.Event) : null;
     var waitList = this._toWaitList(event_wait_list);
     var err = this.cl.imports.clEnqueueSVMFree(this.handle, num_svm_pointers, svm_pointers, null, waitList.count, waitList.array, event);
     this.cl.checkError(err);
     if (event) {
         return new CLEvent(this.cl, event.deref());
     }
 ...
 CLCommandQueue.prototype.enqueueSVMMemcpy = function(dst_ptr, src_ptr, size, event_wait_list) {
     this._throwIfReleased();
     var event = this.isWaitable ? ref.alloc(this.cl.types.Event) : null;
     var waitList = this._toWaitList(event_wait_list);
     var err = this.cl.imports.clEnqueueSVMMemcpy(this.handle, false, dst_ptr, src_ptr, size, waitList.count, waitList.array, event);
     this.cl.checkError(err);
     if (event) {
         return new CLEvent(this.cl, event.deref());
     }
 ...
 CLCommandQueue.prototype.enqueueSVMMemFill = function(svm_ptr, pattern, pattern_size, size, num_event_in_wait_list, event_wait_list, event) {
     this._throwIfReleased();
     var event = this.isWaitable ? ref.alloc(this.cl.types.Event) : null;
     var waitList = this._toWaitList(events_wait_list);
     var err = this.cl.imports.clEnqueueSVMUnmap(this.handle, svm_ptr, waitList.count, waitList.array, event);
     this.cl.checkError(err);
     if (event) {
         return new CLEvent(this.cl, event.deref());
     }
