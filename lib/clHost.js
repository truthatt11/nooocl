...
function CLHost(version) {
    assert(_.contains(_.values(CLHost.supportedVersions), version), "Argument 'version' is invalid or not supported.");
    ...
    else if (version === CLHost.supportedVersions.cl20) {
        this.cl = new CL20();
    }
    else {
...
// ## createV20
// Creates a host instance for the OpenCL 2.0 runtime if it is supported by the actual hardware.
CLHost.createV20 = function () {
    return new CLHost(CLHost.supportedVersions.cl20);
};
